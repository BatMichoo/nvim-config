...
local function load_all()
  ...
  local files = vim.api.nvim_get_runtime_file('lua/lsp/linters/*.lua', true)
  for _, file in ipairs(files) do
    local mod_name = file:match '([^/]+)%.lua$'
    if mod_name and mod_name ~= 'init' then
      local ok, config = pcall(require, 'lsp.linters.' .. mod_name)
...
function M.get()
  ...
  for _, item in ipairs(load_all()) do
    local name = item.mason or item.linter
...
function M.get_by_filetype()
  ...
  for _, item in ipairs(load_all()) do
    for _, ft in ipairs(item.filetypes or {}) do
      ...
      if not vim.tbl_contains(by_ft[ft], item.linter) then
        table.insert(by_ft[ft], item.linter)
      end
    end
  end
  return by_ft
